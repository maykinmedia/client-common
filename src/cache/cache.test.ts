...
describe("cacheGet", () => {
  ...
  test("should return null when a cache record is invalid", async () => {
    sessionStorage.setItem(
      "test.cache_config.key_prefix.foo",
      "{ INVALID JSON",
    );
  ...
  test("should return the cached complex value", async () => {
    const record: CacheRecord = {
      timestamp: new Date().getTime(),
      value: { foo: "bar" },
    };
    ...
    expect(await cacheGet<{ foo: string }>("foo")).toEqual({ foo: "bar" });
  });
});
...
describe("cacheDelete", () => {
  ...
  test("should ignore non matching cache records if startsWith=true", async () => {
    const record: CacheRecord = {
      timestamp: 0,
      value: "bar",
    };
    ...
    sessionStorage.setItem(
      "test.cache_config.key_prefix.bar#foo",
      JSON.stringify(record),
    );

    await cacheDelete("foo", true);
    expect(
      sessionStorage.getItem("test.cache_config.key_prefix.bar#foo"),
...
describe("cacheMemo", () => {
  ...
  test("should store a key without parameters", async () => {
    await cacheMemo("foo", () => "bar");
