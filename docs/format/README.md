[**@maykin-ui/client-common**](../README.md)

***

[@maykin-ui/client-common](../README.md) / format

# format

## Type Aliases

- [String2TitleOptions](type-aliases/String2TitleOptions.md)

## Variables

- [REGEX\_URL](variables/REGEX_URL.md)

## Functions

- [addSpaces](functions/addSpaces.md)
- [distinctArray](functions/distinctArray.md)
- [forceArray](functions/forceArray.md)
- [hyphens2Whitespace](functions/hyphens2Whitespace.md)
- [isLink](functions/isLink.md)
- [removeSpecialChars](functions/removeSpecialChars.md)
- [slugify](functions/slugify.md)
- [string2Title](functions/string2Title.md)
- [strip](functions/strip.md)
- [stripEnd](functions/stripEnd.md)
- [stripStart](functions/stripStart.md)
- [title](functions/title.md)
- [toASCII](functions/toASCII.md)
- [ucFirst](functions/ucFirst.md)
- [whitespace2Hyphens](functions/whitespace2Hyphens.md)
